<script lang="ts">
  ...
  import {
    getFechasSolicitudes,
    getCierreSolicitudes,
    type FechaSolicitud,
  ...
  function isDateSelected(date: Date): boolean {
    return fechasSeleccionadas.some(
  ...
  function toggleDate(date: Date) {
    ...
    const index = fechasSeleccionadas.findIndex(
      (d) => d.toDateString() === dateStr,
    );

    if (index === -1) {
      fechasSeleccionadas = [...fechasSeleccionadas, date].sort(
        (a, b) => a.getTime() - b.getTime(),
      );
    } else {
      fechasSeleccionadas = fechasSeleccionadas.filter(
        (d) => d.toDateString() !== dateStr,
      );
    }
  }
  ...
  async function handleSubmit() {
    ...
    if (tipoNovedad === "Cita médica" && archivos.length === 0) {
      toast.error("Debes adjuntar al menos un archivo para la cita médica");
      return;
    }
    ...
    try {
      const fechaConcat = fechasSeleccionadas
        .map((f) => formatDateToAPI(f))
        .join(",");
      const fields = {
      ...
      if (archivos.length > 0) {
        await permisosStore.createPermisoConArchivos(
          fields,
...
<div class="px-4 sm:px-6 lg:px-8 py-6 max-w-2xl mx-auto">
  <div class="mb-8">
    <a
      href="/dashboard"
      class="inline-flex items-center gap-2 text-sm text-texto-grey hover:text-primario transition-colors mb-4"
    >
      <svg
        class="w-4 h-4"
        viewBox="0 0 24 24"
    ...
    <h1
      class="font-display text-3xl font-extrabold text-texto-dark tracking-tight"
    >
      Nuevo Permiso
    </h1>
    <p class="text-sm text-texto-grey mt-1">
      Completa el formulario para registrar tu permiso
    </p>
  </div>
  ...
  <div class="bg-white rounded-2xl border border-fondo-soft p-6 mb-6">
    {#if $currentUser}
      <div class="flex items-center gap-4">
        <div
          class="w-14 h-14 rounded-full bg-fondo-soft border-2 border-primario/20 flex items-center justify-center overflow-hidden flex-shrink-0"
        >
          {#if $currentUser.foto}
            <img
        ...
        <div>
          <h2
            class="font-display text-lg font-bold text-texto-dark tracking-tight uppercase"
          >
            {$currentUser.nombre || "Usuario"}
          </h2>
          <div class="flex items-center gap-3 mt-1">
            <span class="text-xs text-texto-grey font-medium">
              {$currentUser.area === "Operaciones"
                ? `Código: ${$currentUser.codigo || "N/A"}`
                : `Cédula: ${$currentUser.cedula || "N/A"}`}
            </span>
            <span class="w-px h-3 bg-texto-grey/30"></span>
            <span class="text-xs text-texto-grey"
              >{$currentUser.cargo || "Sin cargo"}</span
            >
          </div>
        </div>
      </div>
  ...
  {#if cierreConfig?.cerrado}
    <div
      class="bg-white rounded-3xl border border-fondo-soft p-8 text-center shadow-sm animate-fade-in"
    >
      <div
        class="w-16 h-16 bg-amber-50 rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-sm"
      >
        <svg
      ...
      <h2
        class="font-display text-xl font-extrabold text-texto-dark leading-snug tracking-tight mb-2"
      >
        {cierreConfig.titulo}
      </h2>
      ...
      {#if cierreConfig.fechaApertura}
        <div
          class="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 text-amber-800 border border-amber-200/40 rounded-xl text-xs font-bold shadow-sm shadow-amber-500/5"
        >
          <svg
            class="w-4 h-4"
            fill="none"
          ...
          Reapertura programada: {formatFechaApertura(
            cierreConfig.fechaApertura,
          )}
        </div>
      {/if}
    ...
    >
        ...
        <div class="relative" data-dropdown>
          ...
          >
            <span class="flex items-center gap-2">
              {tipoNovedad ||
                ($permisosStore.isLoading
                  ? "Cargando..."
                  : "Seleccione el tipo de novedad")}
              {#if fechasSeleccionadas.length >= 2}
                <span
                  class="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded font-bold"
                  >Auto</span
                >
              {/if}
            </span>
            <svg
              class="w-5 h-5 text-texto-grey flex-shrink-0 transition-transform duration-200"
            ...
            >
              {#each $permisosStore.tipos as tipo}
                <button
                  ...
                  class="w-full px-4 py-3 text-left text-sm font-medium transition-colors duration-150 {tipoNovedad ===
                  tipo
                    ? 'bg-primario/10 text-primario'
                    : 'text-texto-dark hover:bg-primario/10 hover:text-primario'}"
                >
                  {tipo}
                </button>
              {/each}
        ...
        <div class="flex flex-col gap-2" data-dropdown>
          ...
          <div class="relative">
            <button
              type="button"
              onclick={() =>
                (showSubPoliticaDropdown = !showSubPoliticaDropdown)}
            ...
            >
              <span
                >{subPoliticaSeleccionada || "Seleccione la sub política"}</span
              >
              <svg
                class="w-5 h-5 text-texto-grey flex-shrink-0 transition-transform duration-200"
                class:rotate-180={showSubPoliticaDropdown}
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
            ...
            {#if showSubPoliticaDropdown}
              ...
              >
                {#each $permisosStore.politicas as politica}
                  <div
                    class="px-4 py-2 bg-fondo-soft text-xs font-bold text-texto-grey uppercase tracking-wider"
                  >
                    {politica.nombre}
                  </div>
                  {#each politica.subpolitica || [] as sub}
                    <button
                      type="button"
                      onclick={() => {
                        subPoliticaSeleccionada = sub.nombre;
                        showSubPoliticaDropdown = false;
                      }}
                      class="w-full px-4 py-2.5 pl-8 text-left text-sm font-medium transition-colors duration-150 {subPoliticaSeleccionada ===
                      sub.nombre
                        ? 'bg-primario/10 text-primario'
                        : 'text-texto-dark hover:bg-primario/10 hover:text-primario'}"
                    >
          ...
          <div class="flex flex-col gap-3 px-1">
            <div class="flex flex-wrap gap-2">
              {#each fechasSeleccionadas as fecha}
                <span
                  class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-primario/10 text-primario rounded-full text-xs font-semibold"
                >
                  {fecha.getDate()}
                  {[
                    "enero",
            ...
            {#if fechasSeleccionadas.length >= 2}
              <div
                class="flex items-start gap-2.5 px-4 py-3 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/80 rounded-xl shadow-sm"
              >
                <div
                  class="flex-shrink-0 w-8 h-8 bg-amber-100 rounded-full flex items-center justify-center"
                >
                  <svg
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xs text-amber-700 leading-relaxed">
                    Has seleccionado <span class="font-bold"
                      >{fechasSeleccionadas.length} días</span
                    >. Este permiso será procesado como licencia no remunerada.
                  </p>
                </div>
              </div>
            {/if}
      ...
      {#if tipoNovedad === "Cita médica"}
        <div class="flex flex-col gap-3">
          ...
          >
            Archivos <span
              class="text-texto-grey/60 font-normal normal-case tracking-normal"
              >({archivos.length}/{MAX_ARCHIVOS}) · Máx 10MB por archivo</span
            >
          </label>
          ...
          >
            ...
            {#if archivos.length >= MAX_ARCHIVOS}
              <svg
                class="w-8 h-8 text-texto-grey/40 mx-auto mb-2"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                stroke-width="1.5"
            ...
            {:else}
              <svg
                class="w-8 h-8 text-primario/50 mx-auto mb-2"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                stroke-width="1.5"
              ...
              <p class="text-sm text-texto-dark font-medium">
          ...
          {#if fileError}
            <p class="text-xs text-error font-medium animate-fade-in">
              {fileError}
            </p>
          {/if}
              ...
              {#each archivos as archivo}
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-texto-dark truncate">
                      {archivo.file.name}
                    </p>
